...
class DecodeBox():
    ...
    def decode_box(self, inputs):
        ...
        for i, input in enumerate(inputs):
            ...
            outputs.append(output.data)#==========最后这里输出的为xywh的真实值
        return outputs
    ...
    def yolo_correct_boxes(self, box_xy, box_wh, input_shape, image_shape, letterbox_image):
        ...
        # pdb.set_trace()
        boxes  = np.concatenate([box_mins[..., 0:1], box_mins[..., 1:2], box_maxes[..., 0:1], box_maxes[..., 1:2]], axis=-1)#===box的shape（10,4）
    ...
    def non_max_suppression(self, prediction, num_classes, input_shape, image_shape, letterbox_image, conf_thres=0.5, nms_thres=0.4):
        ...
        prediction[:, :, :4] = box_corner[:, :, :4]#这里存放的是左上和右下角
        ...
        for i, image_pred in enumerate(prediction):
            ...
            #z这里得到25200个坐标每个坐标中最大的类别概率----torch.max()[0]==最大值----torch.max()[1]最大值的索引
            #=====class_conf.shape(25200,1)-----class_pred.shape(25200,1)
            #----------------------------------------------------------#
            #   利用置信度进行第一轮筛选
            #----------------------------------------------------------#
            ...
            #   7的内容为：x1, y1, x2, y2, 是否存在物体的置信度, 类别概率, 类别标签
            #-------------------------------------------------------------------------#
