...
class YOLOLoss(nn.Module):
    ...
    def box_giou(self, b1, b2):
        ...
        输入为：
        ----------
        b1: tensor, shape=(batch, feat_w, feat_h, anchor_num, 4), xywh====pred_boxes里面是xy的坐标和wh长宽
    ...
    def forward(self, l, input, targets=None, y_true=None):
        ...
        #-----------------------------------------------#
        prediction = input.view(bs, len(self.anchors_mask[l]), self.bbox_attrs, in_h, in_w).permute(0, 1, 3, 4, 2).contiguous()#==>(16,3,16,20,20)===>(16,3,20,20,16)
        ...
        #   作为负样本不合适
        #----------------------------------------------------------------#
        pred_boxes = self.get_pred_boxes(l, x, y, h, w, targets, scaled_anchors, in_h, in_w)#这个函数只是将网络的偏移量预测转换为像素坐标和wh

        if self.cuda:
            y_true          = y_true.type_as(x)
        # pdb.set_trace()
        loss    = 0
        n       = torch.sum(y_true[..., 4] == 1)#真实的框有几个
        if n != 0:
            #---------------------------------------------------------------#
            ...
            loss_loc    = torch.mean((1 - giou)[y_true[..., 4] == 1])#定位损失
            ...
            #self.smoothing==0,self.num_classes ==11====分类损失
            ...
            #这里detach相当于复制张量但是不能回求梯度。clamp(0)的操作相当于将giou中的负值均等于0
        else:
    ...
    def get_pred_boxes(self, l, x, y, h, w, targets, scaled_anchors, in_h, in_w):
        ...
        #   生成网格，先验框中心，网格左上角
        #-----------------------------------------------------#
        grid_x = torch.linspace(0, in_w - 1, in_w).repeat(in_h, 1).repeat(#type_as数据类型转换
            int(bs * len(self.anchors_mask[l])), 1, 1).view(x.shape).type_as(x)#====grid_x（16，3，20，20）
...
def get_lr_scheduler(lr_decay_type, lr, min_lr, total_iters, warmup_iters_ratio = 0.05, warmup_lr_ratio = 0.1, no_aug_iter_ratio = 0.1, step_num = 10):
